module SourceData
  ...
  def get_source_data(metric, start_time, end_time, granularity)
    if metric['operator']
      operands_data = metric['operands'].map do |operand|
        get_source_data(operand, start_time, end_time)
      end
      method_name = 'apply_operator_' + metric['operator']
      source_data = SourceData.send(method_name, operands_data)
    else
      method_name = 'get_source_data_from_' + metric['source']
      source_data = SourceData.send(method_name, metric, start_time, end_time, granularity)
    end
    source_data
  end

  def get_source_data_from_librato(metric, start_time, end_time, granularity)
    intervals = divide_interval_for_librato(start_time, end_time, granularity)
    source_data = []
    intervals.each do |interval_start, interval_end|
      ...
      url += "&resolution=#{eval('1.' + granularity)}"
  ...
  def divide_interval_for_librato(start_time, end_time, granularity)
    """
    librato only permits queries that return 100 elements at most
    if granularity is hours (for example)
    ...
    while interval_start < end_time
      interval_end = interval_start + eval('100.' + granularity)
  ...
  def apply_operator_division(operands_data)
    numerator_data, denominator_data = operands_data
    numerator_data.zip(denominator_data).map do |num, den|
      {'x' => num['x'], 'y' => den['y'] == 0 ? 0 : num['y'] / den['y']}
    end
  end
